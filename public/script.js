        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (bColumnValue === maDonhangURI) {
                ...
                orderDetails = {
                    ...
                    cong: row[26] || '', // Cột AA
                    mucChietkhau: row[27] || '', // Cột AB
                    giatriChietkhau: row[28] || '', // Cột AC
                    phiVanchuyenlapdat: row[29] || '', // Cột AD
                    mucthueGTGT: row[30] || '', // Cột AE
                    thueGTGT: row[31] || '', // Cột AF
                    tamUng: row[32] || '', // Cột AG
                    sotienPhaithanhtoan: row[33] || '', // Cột AH
                };
...
function processFoundData(orderDetails) {
    ...
    const sotienConthieu = formatNumber(orderDetails.sotienPhaithanhtoan) - formatNumber(orderDetails.tamUng);
    ...
    toggleRowVisibility('rowChietKhau', orderDetails.giatriChietkhau);
    toggleRowVisibility('rowPhiVanChuyen', orderDetails.phiVanchuyenlapdat);
    toggleRowVisibility('rowThueGTGT', orderDetails.thueGTGT);
    toggleRowVisibility('rowTamUng', orderDetails.tamUng);
...
function displayConditions() {
    ...
    const phuongThucban = orderDetails.phuongThucban || '';
    const donviPhutrach = orderDetails.donviPhutrach || '';
    const thueGTGT = orderDetails.thueGTGT || '';
    const phiVanchuyenlapdat = orderDetails.phiVanchuyenlapdat || '';
    ...
    if ((phiVanchuyenlapdat === "" || phiVanchuyenlapdat === "0") && phuongThucban !== "Bán lẻ") {
        ...
        if (!item.maDonhangCT.includes("1C.029.01")) {
            tableBody.innerHTML += `
                <tr class="bordered-table">
                    ...
                    <td class="borderedcol-11">${item.dongia || ''}</td>
                    <td class="borderedcol-12">${item.giaban || ''}</td>
                </tr>
            `;
        } else {
            tableBody.innerHTML += `
